...
class Processor:
    ...
    def update_database(self):
        # get current state of database table
        ...
        # get current state from service manager
        live_services = self.client.list()

        # get current state from googleapis/googleapis
        googleapis_protos = get_published_services()

        counter = 0
        for s in live_services:
            if s not in db_services:
                counter += 1
                if s in googleapis_protos:
                    in_googleapis = True
                else:
                    in_googleapis = False
                service_config = self.client.get(s)
                if service_config is not None:
                    if service_config.get("title"):
                        title = service_config["title"]
                        is_cloud = self.is_cloud_service(service_config)
                        new_db_services.append((s, title, is_cloud, in_googleapis, True))
